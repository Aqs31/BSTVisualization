...
public class BSTVisualization extends JFrame implements ActionListener, KeyListener {
    ...
    private ArrayList<Node> searchPath; // List to store nodes in the search path
    ...
    public BSTVisualization() {
        ...
        labelSearch.setFont(new Font("Arial", Font.PLAIN, 18));
        infoPanel.add(labelSearch);

        searchPath = new ArrayList<>(); // Initialize the search path list
        ...
        if (tf.isEnabled()) {
            try {
                ...
                if (evt.getSource() == btnAdd) {
                    ...
                    search(Integer.parseInt(tf.getText()));
                    tf.setText("");
                    tf.requestFocusInWindow();
                }else {
                    delete(data);
    ...
    public void search(int data) {
        labelSearch.setText(""); // Reset the search result label

        // Clear the previous search path highlighting
        clearSearchPathHighlight();

        Node searchResult = searchNode(root, data);

        if (searchResult != null) {
            labelSearch.setText(data + " found!");
            highlightSearchPath(searchPath);
        } else {
            labelSearch.setText(data + " not found!");
        }
    }

    // Helper method to search for a node in the BST and store the search path
    ...
    // Helper method to clear search path highlighting
    private void clearSearchPathHighlight() {
        for (Node node : searchPath) {
            node.data.setBackground(Color.green);
        }
        searchPath.clear();
    }

    // Helper method to highlight the search path
    private void highlightSearchPath(ArrayList<Node> path) {
        for (Node node : path) {
            node.data.setBackground(Color.yellow);
        }
    }
